...
fun ActivityScreen(
    ...
    val back = System.currentTimeMillis() - (activity?.endTime ?: 0)
    Scaffold(
        topBar = {
            ActivityTapBar(
                a = activity,
            ...
            Column(
                ...
                Column {
                    ...
                    Row(horizontalArrangement = Arrangement.SpaceAround) {
                        Text("Старт ${activity?.startTime} км", style = Typography.titleMedium)
                        Text("|", style = Typography.titleMedium)
                        Text("Финиш${activity?.endTime} км", style = Typography.titleMedium)
                    }
                }
                Box(modifier = Modifier.border(2.dp, Color.DarkGray)) {
                    Text("${activity?.comment}", style = Typography.labelMedium)
                }
            }
        }
    )
